...
it against any system you do not own. The release of this framework is legal in
the author's jurisdiction. This tool is explictly only designed for students and
authorized red-team operators. Only them.
...
- **Async BOF + Sleep Mask Integration: Async BOFs run while beacon is Ekko-masked via BADGER_FORCE_SLEEP/WAKEUP_SEND/WAKEUP_EXIT**
...
Configuration:
  ...
  server/operators.json        - Operator accounts (auto-generated on first run)
  Tools:
...
BOF (BofDev/shellcode_loader.c): ~500 bytes compiled
